...
//Описывает логику и пользовательский интерфейс лдной карточки с товаром
  ...
  return (
    <article className={`card ${isCategoryClose ? "card_disabled" : ""}`}>
      ...
      <h2 className="card__title">
        {/* Реализуем маршрутизацию для перехода на страницу с детальной информацией о товаре */}
        <Link
        // to- обязательный атрибут указывакм  адресс по которому будет отображение контента
          to={`/pizza/${id}`}
