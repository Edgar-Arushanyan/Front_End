...
export const itemsSlice = createSlice ({
    ...
    reducers: {
        ...
        setItem( state = initialState, action ) {
            ...
            // в формате ключ/значение для оптимизации работы приложнния 
    ...
    extraReducers: {
        //createAsyncThunk даёт доступ к трём сушностям для детализации ассинхронного запроса 
        // 1.Peding => когда ассинхронный запрос в процессе
        [ fetchItems.pending.type]: (state) => {
            state.status = 'loading';
            state.items = [];
        },
        // 2.когда ассинхронный запрос выполнен успешно
