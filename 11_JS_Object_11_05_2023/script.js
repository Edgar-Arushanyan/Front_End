...
function statsHandler() {
    ...
    const minPrice =
    shoppingCart.items.sort((a, b) => a.price - b.price)[0].price;
                    
    const maxPrice =
     shoppingCart.items.sort((a, b) => b.price - a.price)[0].price;

    const totalQuantity =
     shoppingCart.items.reduce((total, el) => total + el.quantity, 0);

    const totalCost =
     shoppingCart.items.reduce((total, el) => total + el.quantity * el.price, 0);
  ...
  statsContainer.innerHTML = `
    ...
    //const maxPrice = shoppingCart.items.reduce((a, b) => a.price - b.price)[shoppingCart.items.length -1].price;
